        ...
        )
{
         ...
         i += gridDim.x * nthds_per_cta)
    {
        ...
        if (index == -1)
        {
            nmsedClasses[i] = -1;
            nmsedScores[i] = 0;
            for (int ii = 0; ii < boxDims; ii++){
              nmsedBoxes[i * boxDims + ii] = 0;
            }
            // nmsedBoxes[i * 4] = 0;
            // nmsedBoxes[i * 4 + 1] = 0;
            // nmsedBoxes[i * 4 + 2] = 0;
        ...
        else
        {
            ...
            const int bboxId = ((shareLocation ? (index % numPredsPerClass)
                        : index % (numClasses * numPredsPerClass)) + bboxOffset) * boxDims;
            nmsedClasses[i] = (index % (numClasses * numPredsPerClass)) / numPredsPerClass; // label
            nmsedScores[i] = score;                                                        // confidence score
            // clipped bbox xmin
            nmsedBoxes[i * boxDims] = clipBoxes ? max(min(bboxData[bboxId],
                        T_BBOX(1.)), T_BBOX(0.)) : bboxData[bboxId];
            // clipped bbox ymin
            nmsedBoxes[i * boxDims + 1] = clipBoxes ? max(min(bboxData[bboxId + 1],
                        T_BBOX(1.)), T_BBOX(0.)) : bboxData[bboxId + 1];
            // clipped bbox xmax
            nmsedBoxes[i * boxDims + 2] = clipBoxes ? max(min(bboxData[bboxId + 2],
                        T_BBOX(1.)), T_BBOX(0.)) : bboxData[bboxId + 2];
            // clipped bbox ymax
            nmsedBoxes[i * boxDims + 3] = clipBoxes ? max(min(bboxData[bboxId + 3],
                        T_BBOX(1.)), T_BBOX(0.)) : bboxData[bboxId + 3];

            for (int ii = 4; ii < boxDims; ii++){
              nmsedBoxes[i * boxDims + ii] = bboxData[bboxId + ii];
            }
    ...
    )
{
    ...
    gatherNMSOutputs_kernel<T_BBOX, T_SCORE, BS><<<GS, BS, 0, stream>>>(shareLocation, numImages, numPredsPerClass,
                                                                           numClasses, boxDims, topK, keepTopK,
