    ...
    void* nmsedScores, void* nmsedClasses, void* workspace, bool isNormalized, bool confSigmoid, bool clipBoxes)
{
    ...
    /*
     * bboxDataRaw format:
     * [batch size, numPriors (per sample), numLocClasses, boxDims]
     */
    // float for now
    void* bboxData;
    size_t bboxPermuteSize = detectionForwardBBoxPermuteSize(shareLocation, N, perBatchBoxesSize, 
        DataType::kFLOAT);
    void* bboxPermute = nextWorkspacePtr((int8_t*) bboxDataRaw, bboxDataSize);

    /*
     * After permutation, bboxData format:
     * [batch_size, numLocClasses, numPriors (per sample) (numPredsPerClass), boxDims]
     * This is equivalent to swapping axis
     */
    if (!shareLocation)
    {
        status = permuteData(
            stream, locCount, numLocClasses, numPredsPerClass, boxDims, DataType::kFLOAT, false, 
    ...
    bool flipXY = true;
    // NMS
    status = customAllClassNMS(stream, N, numClasses, numPredsPerClass, boxDims, topK, iouThreshold, shareLocation, isNormalized,
        DataType::kFLOAT, DataType::kFLOAT, bboxData, scores, indices, postNMSScores, postNMSIndices, flipXY);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
    ...
    // Gather data from the sorted bounding boxes after NMS
    status = gatherNMSOutputs(stream, shareLocation, N, numPredsPerClass, numClasses, boxDims, topK, keepTopK, DataType::kFLOAT,
        DataType::kFLOAT, indices, scores, bboxData, keepCount, nmsedBoxes, nmsedScores, nmsedClasses, clipBoxes);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
