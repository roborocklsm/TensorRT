...
__global__ void customAllClassNMS_kernel(
    const int num,
...
#pragma unroll
        for (int t = 0; t < TSIZE; t++)
        {
            ...
            if (item_idx < max_idx)
            {
                ...
                if (loc_bboxIndex[t] != -1)
                {
                    ...
                    loc_bbox[t].xmin = flipXY ? bbox_data[bbox_data_idx * boxDims + 1]
                      : bbox_data[bbox_data_idx * boxDims + 0];
                    loc_bbox[t].ymin = flipXY ? bbox_data[bbox_data_idx * boxDims + 0]
                      : bbox_data[bbox_data_idx * boxDims + 1];
                    loc_bbox[t].xmax = flipXY ? bbox_data[bbox_data_idx * boxDims + 3]
                      : bbox_data[bbox_data_idx * boxDims + 2];
                    loc_bbox[t].ymax = flipXY ? bbox_data[bbox_data_idx * boxDims + 2]
                      : bbox_data[bbox_data_idx * boxDims + 3];
        ...
        while ((ref_bbox_idx != -1) && ref_item_idx < max_idx)
        {
            ...
            ref_bbox.xmin = flipXY ? bbox_data[ref_bbox_idx * boxDims + 1] : bbox_data[ref_bbox_idx * boxDims + 0];
            ref_bbox.ymin = flipXY ? bbox_data[ref_bbox_idx * boxDims + 0] : bbox_data[ref_bbox_idx * boxDims + 1];
            ref_bbox.xmax = flipXY ? bbox_data[ref_bbox_idx * boxDims + 3] : bbox_data[ref_bbox_idx * boxDims + 2];
            ref_bbox.ymax = flipXY ? bbox_data[ref_bbox_idx * boxDims + 2] : bbox_data[ref_bbox_idx * boxDims + 3];
...
pluginStatus_t customAllClassNMS_gpu(
    cudaStream_t stream,
    const int num,
    const int num_classes,
    const int num_preds_per_class,
    ...
    bool flipXY = false)
{
#define P(tsize) customAllClassNMS_kernel<T_SCORE, T_BBOX, (tsize)>

    void (*kernel[8])(const int, const int, const int, const int, const int, const float,
    ...
    kernel[t_size - 1]<<<GS, BS, BS * t_size * sizeof(bool), stream>>>(num, num_classes, num_preds_per_class, boxDims,
...
// customAllClassNMS LAUNCH CONFIG
...
static std::array<nmsLaunchConfigSSD, 1> nmsSsdLCOptions = {
    nmsLaunchConfigSSD(DataType::kFLOAT, DataType::kFLOAT, customAllClassNMS_gpu<float, float>)};

pluginStatus_t customAllClassNMS(cudaStream_t stream,
                        const int num,
                        const int num_classes,
                        const int num_preds_per_class,
                        const int boxDims,
                        ...
                        bool flipXY)
{
    nmsLaunchConfigSSD lc = nmsLaunchConfigSSD(DT_SCORE, DT_BBOX, customAllClassNMS_gpu<float, float>);
